...
import { NUMBER_OF_COLUMNS, LOCAL_STORAGE_KEY_PREFIX, DEBOUNCE_DELAY } from '../utils/constants';
import {
  fetchPagesEndpoint,
  fetchPageEndpoint,
  createPageEndpoint,
...
export const usePages = (baseUrl: string) => {
  ...
  const syncPagesWithApi = useCallback(async () => {
    try {
      ...
      setState(prev => ({
        ...prev,
        loading: false,
        error: error instanceof Error ? error.message : 'Failed to sync pages with API',
      }));
    }
  }, [baseUrl]);
  ...
  const createPage = useCallback(async (page: Page) => {
    try {
      setState(prev => ({ ...prev, loading: true, error: null }));
  ...
  const updatePage = useCallback(async (page: Page) => {
    try {
      ...
      setState(prev => ({
        ...
        currentPage: prev.currentPage?.key === page.key ? updatedPage : prev.currentPage,
        loading: false,
      }));
  ...
  const deletePage = useCallback(async (key: string) => {
    try {
      setState(prev => ({ ...prev, loading: true, error: null }));
  ...
  const addRowToCurrentPage = useCallback((rowIndex?: number) => {
    setState(prev => {
      if (!prev.currentPage) return prev;
  ...
  const removeRowFromCurrentPage = useCallback((rowId: string) => {
    setState(prev => {
      if (!prev.currentPage) return prev;
  ...
  const addComponentToCurrentPage = useCallback((component: ContentItem) => {
    setState(prev => {
      if (!prev.currentPage) return prev;
  ...
  const updateComponentInCurrentPage = useCallback((componentId: string, updates: Partial<ContentItem>) => {
    setState(prev => {
      if (!prev.currentPage) return prev;
  ...
  const removeComponentFromCurrentPage = useCallback((componentId: string) => {
    setState(prev => {
      if (!prev.currentPage) return prev;
      
      const components = prev.currentPage.components.filter(c => c.id !== componentId);
      const updatedPage = { ...prev.currentPage, components };
      const updatedPages = prev.pages.map(p => 
        p.key === prev.currentPage!.key ? updatedPage : p
      );
