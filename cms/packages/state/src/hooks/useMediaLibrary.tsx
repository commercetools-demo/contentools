...
import { MediaLibraryResult, MediaFile } from '@commercetools-demo/cms-types';
...
export const useMediaLibrary = () => {
  ...
  const updateFileInState = useCallback((fileUrl: string, updates: Partial<MediaFile>) => {
    setState(prev => ({
      ...prev,
  ...
  // Selectors
  const getFileByUrl = useCallback((url: string) => {
    return state.files.find(file => file.url === url) || null;
  }, [state.files]);

  const getFilesByType = useCallback((isImage: boolean) => {
