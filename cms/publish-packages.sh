...
PACKAGE_NAMES=(
    ...
    "@commercetools-demo/contentools-content-type-code-editor"
    ...
    "@commercetools-demo/contentools-property-editor-wisiwyg"
...
build_packages() {
    ...
    if [ ! -f "package.json" ]; then
        echo -e "${RED}❌ Error: package.json not found in cms directory${NC}"
        exit 1
    fi
...
main() {
    echo -e "${BLUE}===========================================${NC}"
    echo -e "${BLUE}   CMS Package Publication Script${NC}"
    echo -e "${BLUE}===========================================${NC}"
