...
const CONTENT_TYPE_CONTAINER = process.env.CONTENT_TYPE_CONTAINER || 'content-type';

const contentTypeController = new CustomObjectController(CONTENT_TYPE_CONTAINER);
...
contentTypeRouter.get('/content-type', async (req, res, next) => {
  ...
  contentTypeRouter.get('/content-type/:key', async (req, res, next) => {
    try {
      ...
      logger.error(`Failed to get content-type object with key ${req.params.key}:`, error);
      next(error);
    }
  });
  ...
  contentTypeRouter.post('/content-type/:key', async (req, res, next) => {
    try {
      ...
      try {
        ...
        if (objectExists) {
            return res.status(400).json({ error: 'content-type object with key already exists' });
          }
      } catch (error) {
