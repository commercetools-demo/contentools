...
export class SchemaBuilder extends LitElement {
  ...
  render() {
    ...
    const propertyEntries = Object.entries(this.schemaObject)
    ...
    return html`
      <div class="schema-builder">
       ...
       <ui-button size="small" @click=${this._toggleTypeSelector}>Add Property</ui-button>
        ...
        <ul class="schema-list" @dragover=${this._handleDragOver}>
          ${propertyEntries.map(([key, schema]) => html`
            <li class="schema-item ${this._draggedItem === key ? 'dragging' : ''}"
                draggable="true"
              ...
              <div class="actions">
                ...
                <button class="delete-btn" @click=${() => this._deleteProperty(key)}>Delete</button>
              </div>
            </li>
          `)}
        </ul>
      ...
      ${this._showTypeSelector 
        ? html`
          ...
          ${this._propertyTypeOptions.map(type => html`
  ...
  private _addNewProperty(type: string) {
    ...
    const maxOrder = Object.values(this.schemaObject)
      .reduce((max, schema) => Math.max(max, schema.order ?? 0), 0);
  ...
  private _handleDrop(e: DragEvent, targetKey: string) {
    ...
    const sortedEntries = Object.entries(this.schemaObject)
      .sort(([, a], [, b]) => (a.order ?? Infinity) - (b.order ?? Infinity));
    ...
    if (sourceIndex > targetIndex) {
      ...
      // Moving down
      const targetOrder = sortedEntries[targetIndex][1].order ?? 0;
      const nextOrder = targetIndex < sortedEntries.length - 1 ? 
        (sortedEntries[targetIndex + 1][1].order ?? 0) : targetOrder + 20;
  ...
  private _ensureOrderValues() {
    ...
    const needsOrderValues = Object.values(this.schemaObject)
      .some(schema => schema.order === undefined);
